...
var enteredweakness = prompt('Enter the weaknesses');                      // The prompt() method will take Pokemon Weakness as input. 
for (x = 0; x < Pokemon.pokemon.length; x++) {                             // for() loop , loop through json dataset.

    for (y = 0; y < Pokemon.pokemon[x].weaknesses.length; y++) {           // for() loop , loop through a weakness array in all pokemons.
        if (Pokemon.pokemon[x].weaknesses[y] == enteredweakness) {         // if() condition checks for is entered weakness is equal to weakness of current pokemon loop.
            alert(Pokemon.pokemon[x].name);
